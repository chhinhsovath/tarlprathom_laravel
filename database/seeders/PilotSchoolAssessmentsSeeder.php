...
class PilotSchoolAssessmentsSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $this->command->info("Creating assessments for students in {$pilotSchools->count()} pilot schools...");
        ...
        $totalStudents = Student::whereNotNull('pilot_school_id')->count();
        $this->command->info("Found {$totalStudents} students in pilot schools");
        ...
        foreach ($pilotSchools as $school) {
            ...
            foreach ($students as $student) {
                ...
                if ($processedCount % 100 === 0) {
                    $this->command->info("  Processed {$processedCount}/{$totalStudents} students, created {$createdCount} assessments");
                }
            }
            
            $schoolStudentCount = $students->count();
            $schoolAssessmentCount = $schoolStudentCount * count($cycles) * count($subjects);
            $this->command->info("  Completed {$school->school_name}: {$schoolStudentCount} students × {$schoolAssessmentCount} assessments");
        }

        $this->command->info("Assessment seeding completed!");
        $this->command->info("Created {$createdCount} assessments for {$processedCount} students across {$pilotSchools->count()} pilot schools");
    }
