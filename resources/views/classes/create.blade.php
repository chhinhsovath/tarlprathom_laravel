<x-app-layout>
                    ...
                    <form method="POST" action="{{ route('classes.store') }}">
                        ...
                        <div class="mt-4">
                            ...
                            <select id="grade_level" name="grade_level" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                <option value="4" {{ old('grade_level') == 4 ? 'selected' : '' }}>
                                    {{ __('Grade') }} 4
                                </option>
                                <option value="5" {{ old('grade_level') == 5 ? 'selected' : '' }}>
                                    {{ __('Grade') }} 5
                                </option>
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="school_id" name="pilot_school_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required onchange="loadTeachers()">
                                ...
                                @foreach($schools as $school)
                                    <option value="{{ $school->id }}" {{ old('pilot_school_id') == $school->id ? 'selected' : '' }}>
                                        {{ $school->school_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="teacher_id" name="teacher_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ old('teacher_id') == $teacher->id ? 'selected' : '' }} data-school="{{ $teacher->school_id }}">
                                        {{ $teacher->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="academic_year" name="academic_year" type="text" class="mt-1 block w-full" :value="old('academic_year', date('Y') . '-' . (date('Y') + 1))" placeholder="{{ __('2024-2025') }}" />
    ...
    <script>
        function loadTeachers() {
            ...
            const allOptions = teacherSelect.querySelectorAll('option[data-school]');
            
            allOptions.forEach(option => {
                if (schoolId && option.dataset.school !== schoolId) {
            ...
            // Reset selection if current teacher is not from selected school
            if (teacherSelect.value) {
                const selectedOption = teacherSelect.querySelector(`option[value="${teacherSelect.value}"]`);
                if (selectedOption && selectedOption.dataset.school !== schoolId) {
                    teacherSelect.value = '';
                }
            }
        }
        
        // Load teachers on page load if school is already selected
