    ...
    <div class="bg-white p-6 rounded-lg shadow">
        ...
        <div class="mb-6 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('Chart Type') }}</label>
                    <select wire:model.live="chartType" class="w-full rounded-md border-gray-300 shadow-sm">
                        <option value="studentsBySchool">{{ __('Students by School') }}</option>
                        <option value="assessmentsByLevel">{{ __('Assessments by Level') }}</option>
                        <option value="mentoringTrend">{{ __('Mentoring Trend') }}</option>
                        <option value="studentProgress">{{ __('Student Progress') }}</option>
                    </select>
                </div>
                
                @if($chartType === 'assessmentsByLevel')
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('Filter by School') }}</label>
