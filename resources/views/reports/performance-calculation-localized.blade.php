    ...
    <div class="w-full px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-2xl font-bold text-gray-900">{{ trans_db('Performance Calculation Report') }}</h2>
                    <a href="{{ route('reports.index') }}" class="text-sm text-gray-600 hover:text-gray-900">
                        ← {{ trans_db('Back to Reports') }}
                    </a>
                </div>
                <p class="text-gray-600 mb-4">{{ trans_db('Analysis of student performance based on reading levels and mathematical operations capability') }}</p>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-8 bg-white border-b border-gray-200">
                <h3 class="text-xl font-medium text-gray-900 mb-6">{{ trans_db('Calculation of Performance') }}</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-green-50 p-6 rounded-lg">
                        <h4 class="font-semibold text-green-900 mb-3 text-lg">{{ trans_db('b. Math -') }}</h4>
                        <div class="text-base text-green-800 space-y-2">
                            <div class="ml-4">
                                <span class="font-medium">i.</span> {{ trans_db('Students who can do operations = Subtraction + Division + Word Problem') }}
                            </div>
                            <div class="ml-4">
                                <span class="font-medium">ii.</span> {{ trans_db('Beginners = Beginner + 1-Digit') }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-8 bg-white border-b border-gray-200">
                <h3 class="text-xl font-medium text-gray-900 mb-6">{{ trans_db('c. Sample graph') }}</h3>
                
                <!-- Chart container -->
                <div class="bg-yellow-50 p-8 rounded-lg">
                    <h4 class="font-semibold text-center mb-6 text-lg text-gray-800">
                        {{ trans_db('Children who can read at least a simple paragraph') }}
                    </h4>
                    ...
                    <div class="flex justify-center mt-4 space-x-6">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm">{{ trans_db('baseline') }}</span>
                        </div>
                        <div class="flex items-center">
                            <div class="w-4 h-4 bg-orange-600 rounded mr-2"></div>
                            <span class="text-sm">{{ trans_db('midline') }}</span>
                        </div>
                        <div class="flex items-center">
                            <div class="w-4 h-4 bg-blue-600 rounded mr-2"></div>
                            <span class="text-sm">{{ trans_db('endline') }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-8 bg-white">
                <h3 class="text-xl font-medium text-gray-900 mb-6">{{ trans_db('Performance Data - ') . trans_db(ucfirst($cycle)) }}</h3>
                        ...
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-8 py-4 text-center text-sm font-medium text-gray-500 uppercase tracking-wider" colspan="3">
                                    {{ trans_db('Language Performance') }}
                                </th>
                                <th class="px-8 py-4 text-center text-sm font-medium text-gray-500 uppercase tracking-wider" colspan="3">
                                    {{ trans_db('Math Performance') }}
                                </th>
                            </tr>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-12 bg-white text-center">  
                <div class="text-gray-500">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mb-2">{{ trans_db('No Performance Data Available') }}</h3>
                    <p class="text-gray-600">{{ trans_db('No assessment data found for the selected criteria. Please try different filters or ensure assessments have been completed.') }}</p>
                </div>
            </div>
        </div>
        @endif
...
const chartTranslations = {
    'cycle': @json(trans_db(ucfirst($cycle)))
};
